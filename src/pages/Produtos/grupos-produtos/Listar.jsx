...
function ListarGruposDeProdutos() {
    ...
    .then((response) => {
      response.data["data"].forEach((element) => {
        var array = [
          ...
          element["porcentagem_lucro"].map((item) => {return <Chip className="table-tag" label={item.descricao+' '+item.porcentagem+'%'} color="primary" variant="outlined"/>}),
  ...
  return (
    <>
      ...
      <Button onClick={() => history.push("/grupos-produtos/novo")} variant="outlined" startIcon={<AddIcon />} className={'btn btn-primary btn-spacing'}>
        Adicionar
      </Button>
